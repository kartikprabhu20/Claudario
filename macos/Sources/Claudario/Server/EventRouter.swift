...
final class EventRouter {
    ...
    private let onHookActivity: () -> Void
    ...
    init(onWalk: @escaping () -> Void,
         ...
         onHookActivity: @escaping () -> Void) {
        self.onWalk = onWalk
        self.onIdle = onIdle
        self.onCelebrate = onCelebrate
        self.onNotify = onNotify
        self.onActivity = onActivity
        self.onHookActivity = onHookActivity
    }
    ...
    private func process(event: HookEvent, sessionId: String, toolName: String?) {
        ...
        evictStaleLocked()
        defer { lock.unlock() }
        // Every hook event is a signal that the statusline likely just
        // rewrote ~/.claudario/usage.json; refresh the bars off the lock.
        defer { DispatchQueue.main.async { self.onHookActivity() } }
